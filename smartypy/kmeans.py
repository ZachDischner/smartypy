...
TODO/Improvements:
    ...
    * Slow. Some parts are easy to parallelize though without losing the overall class 
        structure and easiness to follow along
...
def initialize_centroids(X,num):
    ...
    return [Centroid(x) for x in X[rand_indices[0:num]]]
...
def associate_centroids(X,centroids):
    ...
    for x in X:
        ...
        centroids[np.argmin(distances)].associate(x)
    return centroids
...
def classify_kmeans(X,num,max_iters=10,min_cohesion=0.001):
    ...
    for ii in range(max_iters):
        print("\rClassification iteration {}, %{:3.2f} Finished".format(ii,float(ii/max_iters)*100), end="")
        centroids = associate_centroids(X,centroids)
        
        ## Dinky check for convergence
        if ii > 0:
    ...
    print("Classification finished with overall average cohesion {:3.3f}".format(overall_cohesion))
    return centroids
